...
_SCHEMA_VERSIONS = frozenset(("0.0.2",))
_LATEST_SCHEMA_VERSION = max(_SCHEMA_VERSIONS)


def validate_json_schema(
    json_dict: Dict[Text, Any], schema_version: Text = None
) -> Dict[Text, Any]:
    """Validates the json schema of a model card field.

    If schema_version is not provided, it will use the latest schema version.
    See
    https://github.com/model-card/model-card/tree/master/model_card_toolkit/schema/.
    ...
    Raises
    ------
      ValueError: If `schema_version` does not correspond to a model card
          schema version.
      ValidationError: If `model_card_json` does not follow the model card
          schema.
    """
    schema = _find_json_schema(
        schema_version
        or json_dict.get("schema_version")
...
def _find_json_schema(schema_version: Text = None) -> Dict[Text, Any]:
    ...
    schema_file = os.path.join(
        "schema", "v" + schema_version, _SCHEMA_FILE_NAME
    )
